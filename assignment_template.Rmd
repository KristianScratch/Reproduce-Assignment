...
A surprisingly common graphical miscommunication in academic literature is connecting discrete data points with a continuous line. So I have used this design floor here to show how it can be misleading. The connecting line suggests that there are values between the point plotted that do (or on average will) fall on the line (Franzblau & Chung 2012). Often times these 'between values' can just not be known (eg if a survey question discrete age groups of 20-30, and 30-40), but in our case this is more extreme as there is no between value possible between these catagories as they are not obviously related, like age is. That is, there is no clear connection between these 3 islands that would suggest a connection as our misleading line does (Durbin 2004). This data IS useful because it could show numbers of penguin according to a property of the island (such as how isolated it is), it could give us information to help conservation or ecotourism on the islands, or even just show biases in the data set. However, using a connected line graph suggests that there is a trend "penguin abundance is increasing", even though we know that this data shows nothing of the sort. 

Furthermore the actual data is pushed to the top of the graph due to bad y limits. This not only makes it harder to read, but also makes the number seem larger. Along with the misleading line and a -though accurate- purposefully misleading title, I would not be surprised if someone in a rush took this to mean there are too many penguins and they are on the rise. It should instead be a bar graph to show averages of categorical variables, which would allow us to add confidence intervals too so that significance can be quickly estimated by eye. A bar graph does not imply connection, but allows comparison which is the valuable part of this data (to see which islands have the most penguins or largest sample bias).

Finally, it actually obscures a lot of information. In general it is bad practice to plot summary statistics (such as mean) without any way to see the raw data (Franzblau & Chung 2012). In this case it is not clear that the Biscoe island is dominated by Gentoo penguins, and Dream island by Chinstrap. This means that, on a fundamental level, the means are no very useful to compare. Showing the underlying data would allow us to colour by species (or other confounding variables) so we can see biases like these and remove the from the data - and, for example, compare just the abundance of Adelie on each island
...
#Install and library the nescesary packages
install.packages(c("ggplot2", "palmerpenguins", "janitor", "dplyr", "car", "ragg", "Hmisc"))
library(ggplot2)
library(palmerpenguins)
library(janitor)
library(dplyr)
...
source('functions/Functions-cleaning.R')
source('functions/Functions-plot.R')
...
# This function will plot the boody mass of the two sexes as a bar graph, including the actual data points with geom_jitter, and 95% confidence intervals 
...
#And then we can group them into a data frame so that we can print it and view them all together.
